...
def driver_options(profile_dir):
    ...
    options.add_extension("driver/CyberGhost_VPN.crx")#crx file path
...
def connect_vpn(driver):
    ...
    time.sleep(1)
    # Select country
    # document.getElementsByClassName('mat-select-trigger')[0].click()
    try: driver.execute_script("document.getElementsByClassName('mat-select-trigger')[0].click()")
    except Exception as e:print(e)
    try: 
        countries_drop_down_btn = driver.find_element(By.TAG_NAME, 'mat-select-trigger')
        countries_drop_down_btn.click() 
    except Exception as e:print(e)
    ...
    #     try:
    #         connected_btn = driver.find_element(By.CLASS_NAME, 'connected')
    #         if connected_btn : break
    #     except Exception as e:... 
    #     print(1)
    
def get_driver(profile_dir='profile_dir',vpn = False):
    
    options,useragent = driver_options(profile_dir)
    # service = Service()
    driver = webdriver.Chrome(ChromeDriverManager().install(), options=options)
    # driver = webdriver.Chrome()
    if vpn :connect_vpn(driver)
    # stealth(driver,
    #         languages=["en-US", "en"],
