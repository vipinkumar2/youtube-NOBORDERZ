            ...
            // for (i = 0; i < filesAmount; i++) {
                if (allowedExtensions.exec(input.files[0]['name'])) {
                    ...
                    reader.onload = function (e) {
                        var img = $('<img />', {
                        'name': "poststs-image",
                        'class': 'upload-image-preview-media',
                        'src': e.target.result,
                        'width': 404,
                        'height': 368
                          });
                    if ($('.preview-media').length > 0) {
                        $('.preview-media').replaceWith(img);
                    }
                    else if ($('.upload-image-preview-media').length > 0) {
                        $('.upload-image-preview-media').replaceWith(img);
                    }
...
function readUrlMultiImages(input) {
    if (input.files && input.files[0]) {
    ...
    for (i = 0; i < filesAmount; i++) {
        ...
        if (allowedExtensions.exec(file['name'])) {
            ...
            reader.onload = function(event) {
                var carousel_img = $($.parseHTML('<div>')).attr('class', "slide"+"-"+i).appendTo($('.slides'));
                $($.parseHTML('<img>')).attr('src', event.target.result).attr('style', "height:100%;width:28vw").appendTo(carousel_img);
