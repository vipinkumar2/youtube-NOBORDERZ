...
async def on_message(message):
    if str(message.channel).endswith("announcements"):
        ...
        tlutili = TelegramUtilities(bot_api_key=os.getenv('TELEGRAM_BOT_API_KEY','1783850886:AAEWsx7h55x_i5r2bAnx_ursVk9cah2kqtc'))
        tlutili.post_message(message.content, os.getenv('TELEGRAM_CHANNEL_ID',-1001450983362))
    else:
...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        client.run(os.getenv('DISCORD_BOT_TOKEN', 'ODIwNTc5NzM5MzMyMzc4NjM0.YE3Obw.BkxvXPR_qeqhBWCYEGUDUVGJUDo'))
